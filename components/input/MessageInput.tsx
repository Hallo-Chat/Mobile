...
const MessageInput = ({ conversationSelected, handleReload }) => {
  ...
  const sendMessage = async () => {
    ...
    socket.current.emit('sendMessage', {
      senderId: user._id,
      // receiverId,
      to: conversationSelected.isGroup ? conversationSelected.name : receiverId,
      text: message,
      isGroup: conversationSelected.isGroup,
    })

    try {
      const res = await axios.post('http://10.0.2.2:4000/api/messages', {
        sender: user._id,
        text: message,
        conversationsId: conversationSelected._id,
        type: 'Text',
      })
      await handleReload()
      setMessage('')
    ...
    <View style={styles.root}>
      <View style={styles.inputContainer}>
        ...
        <MaterialCommunityIcons
          ...
          size={24}
          color={inputIconColor}
          style={styles.icon_end_of_right}
        />
      </View>
